...
# 2.4
def plot_simple_category_bar(df: pd.DataFrame, column: str, out_path: PathLike) -> Path:
    """
    bar-chart по количеству объектов в выбранной категориальной колонке.
    """
    out_path = Path(out_path)
    out_path.parent.mkdir(parents=True, exist_ok=True)

    if column not in df.columns:
        # если колонки нет, рисуем пустой график
        fig, ax = plt.subplots()
