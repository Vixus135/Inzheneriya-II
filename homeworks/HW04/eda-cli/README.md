...
Теперь включает не только CLI-интерфейс, но и **HTTP-сервис на FastAPI** с REST API для оценки качества датасетов.
...
- `top_categories/*.csv` - таблицы с top‑значениями категориальных признаков;
- `category_bar_<column>.png` - bar‑график распределения значений по выбранной категориальной колонке.

---

### Эвристики качества

- `too_few_rows` - в датасете слишком мало строк (например, меньше 1000)
- `too_many_missing` - в одной или нескольких колонках слишком высокая доля пропусков
- `too_many_columns` - слишком много признаков
- `has_constant_columns` - в датасете есть колонки, в которых все значения одинаковы 
- `has_suspicious_id_duplicates` - в колонках, похожих на идентификаторы (например, id, user_id), обнаружены дубликаты
- `has_many_zero_values` - числовые колонки содержат подозрительно много нулей
- `no_numeric_columns` - в датасете отсутствуют числовые признаки
- `no_categorical_columns` - в датасете отсутствуют категориальные признаки
...
- возвращает: `quality_score`, `flags`, `ok_for_model`, `latency_ms`, `dataset_shape`.
...
### 5. `POST /quality-flags-from-csv` – эвристические флаги по CSV-файлу

Эндпоинт принимает CSV-файл, запускает EDA-ядро и возвращает только булевы флаги качества, без расчёта `quality_score`

Внутри:

- читается CSV-файл в pandas.DataFrame;
...
- из результата отбираются только булевы флаги (dict[str, bool]);

- возвращается словарь вида {"flags": ...}.
**Запрос:**
...
```
**Примечание:**
Этот эндпоинт не возвращает `quality_score`, `ok_for_model` и `dataset_shape`, в отличие от `/quality-from-csv`. Он предназначен для получения детализированных эвристических флагов, включая расширенные признаки из HW03
---
