...
# Моя реализация эндпоинта для 2.3.2 (вариант А)
@app.post(
    "/quality-flags-from-csv",
    tags=["quality"],
    summary="Возвращает набор флагов качества по CSV-файлу",
)
async def quality_flags_from_csv(file: UploadFile = File(...)) -> dict[str, dict[str, bool]]:
    """
    Эндпоинт, который принимает CSV-файл, запускает EDA-ядро и возвращает булевы флаги качества.
    """
    start = perf_counter()
    ...
    print(
        ...
        f"flags={flags_bool} latency_ms={latency_ms:.1f} ms"
    )
